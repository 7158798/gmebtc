...
<div id="siteNoty" class="notification-box"><div class="noty_message">【重要通知】： <a id="siteNotyCon" href="/article/16451" >gate.io充值量子链(QTUM)赢取QTUM大奖活动公告</a><a class="close pull-right" href=javascript:;></a></div></div><div class="header clearfix">
    <div class="top-up">
        <div class="top-con">
            <ul class="topprice">
				<li> BTC/CNY : ￥ <span class="topnum">54590.29</span><i class="icon-arrow-up">&uarr;</i> </li>
                <li> ETH/CNY : ￥ <span class="topnum">4578.78</span><i class="icon-arrow-down">&darr;</i> </li>
                <li> LTC/CNY : ￥ <span class="topnum">875.27</span><i class="icon-arrow-down">&darr;</i> </li>
                <li> QTUM/CNY : ￥ <span class="topnum">104.98</span><i class="icon-arrow-down">&darr;</i> </li>
            </ul>
            <div class="qqtel">
...
<script type="text/javascript">
	...
	<div class="main_content acc-m-con">
			...
			<div class="m_title"><h4>邮箱设置</h4></div>
			<div class="sectioncont deposit-sec">
				<p>已绑定邮箱：<b>850******@qq.com</b>。用于安全通知，登录、安全设置时输入。</p>	
			</div>
...
$(document).ready(function (e) {
    ...
    $('#emailbtn').click(function() {
		...
		var email = $('#email').val();
		var mail_test = /^([a-zA-Z0-9\._-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;
		
		if (typeof email == 'undefined' || email == '' || !mail_test.test(email)) {
			noty({text: "您的邮箱无效", type: "warning",layout: "center", theme: 'gateioNotyTheme', modal: true,timeout: 5000,
					buttons: [{addClass: 'btn btn-primary noty-close-btn', text: '关闭', onClick: function ($noty) {$noty.close();}}]
			});
		...
		$.ajax({type:"post",url:"/email",data:json_req,
			success : function(data, textStatus){
				var json_res = data;
				if ( json_res && json_res.result ) {
					self.text(orignialtext + ' (' + currentTime + ')');
					timer.play();
					msg = lang == "en" ? "The Email code has been sent to your mail box." : "邮箱验证码：" + json_res.msg;
					$('#emailcode', form).prop('disabled', false);
				} else {
					 msg = lang == "en" ? "Send Email faild: "+json_res.msg+", please check your email." : "邮件发送失败："+json_res.msg+"，请检查您的邮箱";
					 timer.play();
				}
				noty({text: msg, type: "warning",layout: "center", theme: 'gateioNotyTheme', modal: true,timeout: 5000,
					buttons: [{addClass: 'btn btn-primary noty-close-btn', text: '关闭', onClick: function ($noty) {$noty.close();}}]});
			},
			error : function(){
				timer.play();
				noty({text: '网络错误!', type: "warning",layout: "center",theme: 'gateioNotyTheme', modal: true,timeout: 5000,
					buttons: [{addClass: 'btn btn-primary noty-close-btn', text: '关闭', onClick: function ($noty) {$noty.close();}}]});
			},
			complete : function(){
			}
...
<div class="footer">
    ...
    <div class="fnav">
        ...
        <div class="tail">
            ...
            <div class="vol-all">
                <span>成交量:</span>
				<span> USDT : $<span id="usdtAll">133154137</span> </span>
                <span> BTC : ฿<span id="btcAll">661</span> </span>
                <span> LTC : Ł<span id="ltcAll">2402</span> </span>
                <span> ETH : E<span id="ethAll">31944</span> </span>
            </div>
            <span id="runTime">
                <!--<a href="#">gate.io</a> is powered by <a href="#">gate.io Dev Team</a>-->
                <span class="admin-view"> Elapsed:6.200ms - cnst:4.1;14:0.4;12:0.5;13:1.6; .234 &nbsp; a/e/r </span>
            </span>
        </div>
    </div>
</div>
...
<script>
    ...
    $("#usdtAll").text(toThousands(133154137));
    $("#btcAll").text(toThousands(661));
    $("#ltcAll").text(toThousands(2402));
    $("#ethAll").text(toThousands(31944));
