 ...
 * 
 */
package com.gmebtc.web.portal.sercurity;
...
public class XSSSecurityManager {
    ...
    public static void init(FilterConfig config){
    	...
    	//初始化过滤配置文件
        String xssPath = Thread.currentThread().getContextClassLoader().getResource("").getPath()+"/config/xss_security_config.xml";
	...
	public static boolean initConfig(String path) throws DocumentException {
		...
		logger.info("XSSSecurityManager.initChonfig(Strig patn) end");
		return true;
