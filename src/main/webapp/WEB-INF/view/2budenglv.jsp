...
<script type="text/javascript">
	...
	<div class="main_content acc-m-con">
			...
			<div class="m_title"><h4>两步登陆保护设置</h4></div>
			<div class="sectioncont">
				<p>目前 两步登陆保护 状态为：<b><font color=green>已开启</font></b> </p>
				
				<p>两步登陆保护提供更为安全的用户登陆防护。启用这一功能前，您需要首先开启<a href='/myaccount/sms_setup'>手机验证码功能</a>或<a href='/myaccount/totp'>密码双重验证</a>功能。启用这一功能后，在您每次登陆前都必须输入手机短信验证码或双重验证码，保障您的登陆安全。</p>
		

				<form enctype="application/x-www-form-urlencoded" method="post" action="">		
				<input type="hidden" value="0" size="20" name="login2" id="login2">
				<table  class="login2-table">
				
								
								<tr>
					<td align="right"  width="30%">短信验证码：</td>
					<td  width="70%">
				...
				<tr>
					...
					<td>
					<img id="login2SetupCaptcha" src="/captcha" alt="Captcha" title="看不清？换一张" onclick="document.getElementById('login2SetupCaptcha').src = '/captcha?' + Math.random(); return false"/>
					</td>
				</tr>
				<tr>
					<td align="right">请输入上面的验证码：</td><td><input type="text" name="captcha" id="captcha"  class="login2-captcha"/></td>
				</tr>				
				-->
				<tr>
				<td>&nbsp;</td>
				<td> <input type="submit" name="submit" id="submit" value="  关闭 两步登陆保护 " class="sub-btn"></td>
				</tr>
                <tr>
                    <td></td>
                    <td>
...
$(document).ready(function (e) {
    ...
    $('#voicebtn').click(function() {
        ...
        var resetTimer = function()
        {
			...
			var view_code = "";
			var msg = "";
			$.ajax({type:"post",url:"/voiceverify/" + smsid,
				success : function(data, textStatus){
					var json_res = data;
					if ( json_res && json_res.result ) {
						self.text(orignialtext + ' (' + currentTime + ')');
...
<div class="footer">
    ...
    <div class="fnav">
        ...
        <div class="tail">
            ...
            <div class="vol-all">
                <span>成交量:</span>
				<span> USDT : $<span id="usdtAll">133068183</span> </span>
                <span> BTC : ฿<span id="btcAll">657</span> </span>
                <span> LTC : Ł<span id="ltcAll">2400</span> </span>
                <span> ETH : E<span id="ethAll">31927</span> </span>
            </div>
            <span id="runTime">
                <!--<a href="#">gate.io</a> is powered by <a href="#">gate.io Dev Team</a>-->
                <span class="admin-view"> Elapsed:32.949ms - cnst:0.9;14:0.3;12:0.6;13:1.5; .234 &nbsp; a/e/r </span>
            </span>
        </div>
    </div>
</div>
...
<script>
    ...
    $("#usdtAll").text(toThousands(133068183));
    $("#btcAll").text(toThousands(657));
    $("#ltcAll").text(toThousands(2400));
    $("#ethAll").text(toThousands(31927));
